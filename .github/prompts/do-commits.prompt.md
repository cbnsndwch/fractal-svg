...
- Directories: `src`, `scripts`, `docs`, `output`
- Specific areas: `cli`, `fractals`, `rendering`, `prompts`
- Config: `tsup`, `eslint`, `typescript`
...
```
feat(fractals): add Julia set fractal generator
refactor(cli): optimize interactive prompt flow
fix(rendering): correct gradient application on open paths
build(deps): update tsx and typescript
chore(output): update example SVG files
```
