run `pnpm dlx ncu` to check for outdated packages

if you find any, group them by topic/area so that packages that are related get updated together

for each group, run `pnpm dlx ncu -u -f ...` with the appropriate filters (that's what the `-f` flag is for) to update the version specs in package.json files then `pnpm install` to install the updated packages
