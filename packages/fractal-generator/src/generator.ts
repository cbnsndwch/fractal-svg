...
const FRACTAL_CONFIG: Record<
  FractalType,
  { defaultIter: number; maxIter: number; segmentFormula: string }
> = {
  carpet: {
    defaultIter: 4,
    maxIter: 8,
...
function getIterationConfig(type: FractalType): {
  defaultIter: number;
  maxIter: number;
} {
...
function argsToCliCommand(args: Args): string {
  ...
  if (args.type === "carpet") {
    ...
    if (args.resolution !== 256)
      lines.push(`  --resolution ${args.resolution}`);
...
async function runInteractive(): Promise<Args> {
  ...
  const fractalType = await select({
    ...
    choices: [
      {
        name: "Grid Carpet (Sierpinski-style rectangles)",
        value: "carpet" as const,
      },
      { name: "Koch Curve (Snowflake-style paths)", value: "koch" as const },
      {
        name: "Mandelbrot Set (contour-traced boundary)",
        value: "mandelbrot" as const,
      },
      {
        name: "Julia Set (contour-traced with constant c)",
        value: "julia" as const,
      },
      {
        name: "Dragon Curve (L-system path fractal)",
        value: "dragon" as const,
      },
      { name: "Hilbert Curve (space-filling path)", value: "hilbert" as const },
      {
        name: "Lévy C Curve (feathery symmetric pattern)",
        value: "levy" as const,
      },
      {
        name: "Sierpinski Triangle Arrowhead (line-based triangle)",
        value: "sierpinski" as const,
      },
      {
        name: "Peano Curve (3x3 space-filling curve)",
        value: "peano" as const,
      },
      {
        name: "Gosper Curve/Flowsnake (hexagonal space-filling)",
        value: "gosper" as const,
      },
    ],
  });
  ...
  if (fractalType === "carpet") {
    ...
    const gradientSuffix = gradient
      ? `-grad${gradient.length}c-${gradientAngle}deg`
      : `-${fill.replace("#", "")}`;
...
function parseCLIArgs(argv: string[]): Args | null {
  ...
  if (iter > iterConfig.maxIter) {
    console.warn(
      `⚠️  Iteration ${iter} exceeds max ${iterConfig.maxIter} for ${cmd}. Clamping to ${iterConfig.maxIter}.`,
    );
  ...
  if (cmd === "carpet") {
    ...
    if (!DStr || DStr.startsWith("--")) {
      throw new Error(
        "Carpet mode requires a dimension argument. Example: carpet 1.8928",
      );
    }
    const D = Number(DStr);
    if (!Number.isFinite(D) || D <= 0 || D > 2) {
      throw new Error(
        `Dimension must be a finite number in (0, 2]. Got: ${DStr}`,
      );
    }
    ...
    throw new Error(
      `Unknown command: ${cmd}. Use 'carpet', 'koch', 'mandelbrot', 'julia', 'dragon', 'hilbert', 'levy', 'sierpinski', 'peano', 'gosper', or run without args for interactive mode.`,
    );
  }
}
...
function estimateAutoIterCarpet(
  k: number,
  N: number,
  maxRects: number,
  size: number,
 ...
 */
function generateMandelbrotContours(
  args: MandelbrotArgs,
): Array<Array<[number, number]>> {
 ...
 */
function generateJuliaContours(
  args: JuliaArgs,
): Array<Array<[number, number]>> {
...
function generateCarpet(args: CarpetArgs): void {
  ...
  const iter =
    args.iter ||
    estimateAutoIterCarpet(best.k, best.N, args.maxRects, args.size);
...
function generateJulia(args: JuliaArgs): void {
  console.log(
    `Generating Julia set contours for c = ${args.juliaReal} + ${args.juliaImag}i... (this may take a moment)`,
  );
